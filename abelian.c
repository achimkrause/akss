...
void kernel(int p, struct matrix *f, struct abelian x, struct abelian y, struct matrix_arr to_X, struct matrix_arr from_X, struct abelian *k, struct matrix_arr *to_K, struct matrix_arr *from_K)
{
	...
	struct matrix *rel_free = matrix_init(y.tor_rank + f->width, x.tor_rank); //build from -l and rel_x. ALLOCATES
	...
	struct matrix *f_free = matrix_init(f->height, y.tor_rank + f->width); //build f_free from rel_x, f. ALLOCATES
	...
	struct matrix *f_to_X_i;
	
	struct matrix_arr to_Ry_X;
	matrix_arr_init(&to_Ry_X,to_X.length + 1); //reserve additional entry for rel_free!
	
	struct matrix **to_X_i = to_X.entries;
	struct matrix **to_Ry_X_i = to_Ry_X.entries;
	...
	for(int i=0; i<to_X.length; i++)
	{
		...
		*to_Ry_X_i = matrix_init(y.tor_rank + f->width, (*to_X_i)->width);
		...
		set_submatrix(0,0, y.tor_rank, 0, (*to_X_i)->height, (*to_X_i)->width, (*to_X_i), (*to_Ry_X_i));	
	...
	*to_Ry_X_i = rel_free;
	...
	struct matrix_arr from_Ry_X;
	matrix_arr_init(&from_Ry_X, from_X.length);
	struct matrix **from_Ry_X_i = from_Ry_X.entries;
	struct matrix **from_X_i = from_X.entries;
	for(int i=0; i<from_X.length; i++)
	{
		*from_Ry_X_i = matrix_init((*from_X_i)->height, y.tor_rank+f->width);
		set_submatrix(0,0, 0, y.tor_rank, (*from_X_i)->height, f->width, *from_X_i, *from_Ry_X_i);
	}
	
	struct matrix_arr null_arr;
	null_arr.length = 0;
	...
	while(n<f_free->width && n < f_free->height && !mpq_equal(*entry_f_free,zero))
	{
		n++;
		entry_f_free = entry_f_free + f_free->width + 1;
	}

	struct matrix *rel_K = matrix_init(rel_free->height - n, rel_free->width);
	set_submatrix(n,0, 0,0, rel_K->height, rel_K->width, rel_free, rel_K);


	struct matrix_arr to_free_K;
	matrix_arr_init(&to_free_K, to_Ry_X.length);
	to_Ry_X_i = to_Ry_X.entries;
	struct matrix **to_free_K_i = to_free_K.entries;
	...
	for(int i=0; i<to_Ry_X.length; i++)
	{
		*to_free_K_i = matrix_init(rel_free->height - n, (*to_Ry_X_i)->width);
		set_submatrix(n,0,0,0,(*to_free_K_i)->height, (*to_free_K_i)->width, (*to_Ry_X_i), (*to_free_K_i));
		to_Ry_X_i++;
		to_free_K_i++;
	}
	...
	struct matrix_arr from_free_K;
	matrix_arr_init(&from_free_K, from_Ry_X.length);
	
	from_Ry_X_i = from_Ry_X.entries;
	struct matrix **from_free_K_i = from_free_K.entries;

	for(int i=0; i<from_Ry_X.length; i++)
	{
		*from_free_K_i = matrix_init((*from_Ry_X_i)->height, rel_free->height-n);
		set_submatrix(0,n,0,0,(*from_free_K_i)->height, (*from_free_K_i)->width, (*from_Ry_X_i), (*from_free_K_i));
		from_Ry_X_i++;
		from_free_K_i++;
	}

	struct abelian free_K;
...
{	
	...
	struct matrix *f_rel_y = matrix_init(f->height, f->width + y.tor_rank);
	...
	while(mpq_equal(*entry_diag,one) && n<f_rel_y -> height && n<f_rel_y -> width)
	{
		n++;
	...
	if(c!= NULL)
	{
		...
		int tor_rank_c = 0;
		while(!mpq_equal(*entry_diag,zero) && tor_rank_c<f_rel_y->height - n && tor_rank_c < f_rel_y->width - n)
		{
			tor_rank_c++;
			entry_diag = entry_diag + f_rel_y -> width + 1;
		}
	...
	if(to_C != NULL)
	{
		...
		to_C->entries = malloc(sizeof(struct matrix*) * to_Y.length);

		struct matrix **entry_to_C = to_C->entries;
		struct matrix **entry_to_Y = to_Y.entries;
		
		for(int i=0; i<to_C->length; i++)
		{
			*entry_to_C = matrix_init((*entry_to_Y)->height-n, (*entry_to_Y)->width);
			set_submatrix(n,0,0,0,(*entry_to_C)->height, (*entry_to_C)->width, (*entry_to_Y), (*entry_to_C));
			entry_to_C++;
		}
	}
	...
	if(from_C != NULL)
	{
		...
		from_C->entries = malloc(sizeof(struct matrix*) * from_Y.length);
		
		struct matrix **entry_from_C = from_C->entries;
		struct matrix **entry_from_Y = from_Y.entries;

		for(int i=0; i<from_C->length; i++)
		{
			*entry_from_C = matrix_init((*entry_from_Y)->height, (*entry_from_Y)->width - n);
			set_submatrix(0,n, 0, 0, (*entry_from_C)->height,(*entry_from_C)->width,(*entry_from_Y), (*entry_from_C));
			entry_from_C++;
		}
	}
...
{
	struct matrix *lift = matrix_init(d->width, f->width);
...
int lift(int p, struct matrix *f, struct matrix *g, struct matrix **res)
{
	...
	struct matrix *g_copy;
	struct matrix *f_copy;
	struct matrix *b;
	g_copy = copy(g);
	f_copy = copy(f);
